...
class TARGET_LSTM(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim, sequence_length, start_token, params):
        self.num_emb = num_emb          #369 vocab_size
        self.batch_size = batch_size    #64
        self.emb_dim = emb_dim          #32 each vocab represented by embeddings of size 32
        self.hidden_dim = hidden_dim    #32
        self.sequence_length = sequence_length  #30
        self.start_token = tf.constant([start_token] * self.batch_size, dtype=tf.int32)  #(64,) 64 rows, 1 column
        self.g_params = []              #generator parameters
        self.temperature = 1.0
        self.params = params    #parameters of the lstm model : list of length 15 for each layer

        tf.set_random_seed(66)

        with tf.variable_scope('generator'):
            self.g_embeddings = tf.Variable(self.params[0])     # first layer embedding layer
        ...
        # processed for batch
        with tf.device("/cpu:0"): #converting input batch to embedding
        ...
        self.h0 = tf.stack([self.h0, self.h0])                  #(2 * 64 * 32) But Why stack?
